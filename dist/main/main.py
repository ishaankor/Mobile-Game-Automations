...
def find_current_screen(filename='current_screen.png'):
...
def check_for_errors():
    time.sleep(1)
    try:
        if pyautogui.center(pyautogui.locateOnScreen('connection_error_message.png', confidence=0.9)):
            print("Internet connection error detected!")
            time.sleep(1)
            try_again_button = pyautogui.center(pyautogui.locateOnScreen('try_again_button.png', confidence=0.9))
            time.sleep(1)
        ...
        try:
            if pyautogui.center(pyautogui.locateOnScreen('apology_message.png', confidence=0.9)):
                print("Problem error detected!")
                time.sleep(1)
                continue_button = pyautogui.center(pyautogui.locateOnScreen('continue_button.png', confidence=0.9))
                time.sleep(1)
...
def find_dv_account(filename, confidence_level=0.9, click_option=False):
    while True:
        try:
            ...
            time.sleep(3)
            try:
                deny_load_button = pyautogui.center(pyautogui.locateOnScreen('no_button.png'))
                time.sleep(3)
...
def wait_until_element_appears(filename, confidence_level=0.9, click_option=False):
    if filename != 'free_gift_button.png' and filename != 'friends_button.png':
        find_current_screen('automation_start_screen.png')
    ...
    while automation_status is True:
        if total_time <= 30:
            try:
                print(f"CONFIDENCE LEVEL: {confidence_level} at {time.time() - start_time}")
                ...
                if click_option:
                    ...
                    if filename != 'free_gift_button.png' and filename != 'friends_button.png':
                        time.sleep(5)
                        find_current_screen()
                        if compare_image('automation_start_screen.png', 'current_screen.png'):
                            print("TRUE")
                            automation_status = True
                        else:
                            print("FALSE")
...
def wait_after_loading_screen():
    ...
    while True:
        try:
            x_button = pyautogui.center(pyautogui.locateOnScreen('x_button.png', confidence=0.8))
            time.sleep(3)
            ...
            try:
                collect_button = pyautogui.center(pyautogui.locateOnScreen('collect_button.png'))
                time.sleep(3)
...
def check_screen_after_loading_screen():
    wait_until_element_appears('market_button.png', click_option=False)
    try:
        time.sleep(10)
...
def run_automation(dv_instance, threshold):
    ...
    # time.sleep(5)
    wait_until_element_appears('clear_all_button.png', click_option=True)
    ...
    # time.sleep(3)
    wait_until_element_appears('dragonvale_icon.png', click_option=True)
    # dragonvale_icon = pyautogui.center(pyautogui.locateOnScreen('dragonvale_icon.png', confidence=0.9))
    # time.sleep(3)
    # pyautogui.click(dragonvale_icon)
    ...
    # time.sleep(3)
    wait_until_element_appears("google_sign_in_button.png")
    find_dv_account(dv_instance, confidence_level=threshold, click_option=True)
    wait_until_element_appears('yes_button.png', click_option=True)
...
def gem_farm_account_1():
    ...
    while True:
        try:
            time.sleep(3)
            print("SEARCHING!")
            bluestacks_mi_opener = pyautogui.center(pyautogui.locateOnScreen('bluestacks_mi.png', confidence=0.9))
    ...
    wait_until_element_appears('start_button.png')
    for pos in pyautogui.locateAllOnScreen('start_button.png', confidence=0.9):
...
def gem_farm_account_2():
    ...
    wait_until_element_appears('close_instance_button.png', click_option=True)
    wait_until_element_appears('close_button.png', click_option=True)
    while True:
        try:
            time.sleep(3)
            print("SEARCHING!")
